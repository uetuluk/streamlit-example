...
order = st.sidebar.radio('Order',["total descending",'total ascending'])
...
bar.update_xaxes(range =[-0.5,9.5], categoryorder = order)
st.write(bar)
...
if chart_type =='Choropleth':
  ...
  fig2 = px.scatter_geo(data, locations = 'Country_Code', animation_frame = 'Year', hover_name='Country_Name', projection=view, color='Total_Medals')
  st.write(fig2)